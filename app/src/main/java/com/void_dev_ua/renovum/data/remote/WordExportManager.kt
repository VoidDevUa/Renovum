	...
	fun createWordDocument(
		...
		try {
			...
			headerParagraph.createRun().apply {
				...
				fontSize = 10
				fontFamily = "Arial"
				setText(appContext.getString(R.string.report_disclaimer))
			}
			...
			val notSpecified = appContext.getString(R.string.report_master_not_specified)
			val masterNameText = userSettings.masterName.ifBlank { notSpecified }
			val masterPhoneText = userSettings.masterPhone.ifBlank { notSpecified }

			rightCell.paragraphs[0].apply { alignment = ParagraphAlignment.RIGHT }.createRun().apply {
				fontSize = 11; fontFamily = "Arial"; setText(appContext.getString(R.string.report_master_label, masterNameText))
			}
			rightCell.addParagraph().apply { alignment = ParagraphAlignment.RIGHT }.createRun().apply {
				fontSize = 11; fontFamily = "Arial"; setText(appContext.getString(R.string.report_phone_label, masterPhoneText))
			}
			rightCell.addParagraph().apply { alignment = ParagraphAlignment.RIGHT }.createRun().apply {
				fontSize = 11; fontFamily = "Arial"; setText(appContext.getString(R.string.report_address_label, data.projectName))
			}
			...
			titleParagraph.createRun().apply {
				...
				setText(appContext.getString(R.string.report_title))
			}
			...
			if (isGroupedByRooms) {
				data.roomsWithWorks.forEach { (room, works) ->
					...
					roomParagraph.createRun().apply {
						...
						fontSize = 14
						fontFamily = "Arial"
						setText("\t${appContext.getString(R.string.report_room_total, formatDouble(roomTotal))}")
					}

					buildWorksTable(appContext, document, works)
				}
			} else {
				...
				buildWorksTable(appContext, document, aggregatedWorks)
			}
			...
			if (isGroupedByRooms && data.discountPercent > 0) {
				discountCell.paragraphs[0].createRun().apply {
					...
					setText(appContext.getString(R.string.report_discount_label, formatDouble(data.discountPercent))) }
			}
			...
			totalParagraph.createRun().apply {
				...
				setText(appContext.getString(R.string.report_total_to_pay, formatDouble(data.totalDiscountedSum))) }
	...
	private fun buildWorksTable(
		...
		works.forEachIndexed { index, (applied, service) ->
			...
			val rowData = arrayOf(
				...
				"$pricePerUnitFormatted $currency",
				"$totalWorkPriceFormatted $currency"
			)
